      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-10">
          ...
          <p className="text-muted mt-1 text-sm">
            Real-time overview of your ZINOV store
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {statCards.map((s) => (
            <div
              key={s.label}
              className="bg-white rounded-sm p-6 shadow-sm border border-charcoal/5"
            >
              ...
              {[
                {
                  ...
                  value:
                    stats.totalVisitors > 0
                      ? (
                          (stats.totalSignups / stats.totalVisitors) *
                          100
                        ).toFixed(1) + "%"
                      : "0%",
                },
                {
                  ...
                  value: Math.round(stats.totalVisitors / 30),
                },
                { label: "Active Countries", value: stats.countries.length },
              ].map((item) => (
                <div
                  key={item.label}
                  className="flex justify-between items-center border-b border-cream/10 pb-3"
                >
              ...
              <thead>
                <tr className="border-b border-charcoal/5">
                  {["Name", "Email", "Joined"].map((h) => (
                    <th
                      key={h}
                      className="text-left px-6 py-3 text-xs uppercase tracking-wider text-muted"
                    >
                      {h}
                    </th>
                  ))}
